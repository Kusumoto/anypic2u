...
<center><div class="fb-like-box" data-href="https://www.facebook.com/zxpic" data-width="500" data-height="210" data-show-faces="true" data-stream="false" data-show-border="false" data-header="true"></div></center>
        ...
        <div class="span3">
          <div class="well sidebar-nav">
            <ul class="nav nav-list">
              ...
              <li class="nav-header">Japan Serie</li>
              ...
              <li class="active"><a href="allmovies_m9.php">Browse SitCom</a></li>
  ...
  <p><img src='img/20-0.gif' border='0'>  <strong>หน้านี้จะใช้เวลาโหลดสักครู่นะครับ โปรดรอจนกว่าหน้านี้จะโหลดเสร็จ</strong></p>
           ...
           <div class="text-center"><h3>Browse Sitcom</h3></div>
           <div class="text-right">
               <form class="form-search" action="search_m9.php" method="POST">
                 <input type="text" class="input-medium search-query" placeholder="ชื่อซิตคอมที่ต้องการหา" name="keyword">
          ...
          <?php
          if ($page != 0) {
             echo "<a class='btn' href='allmovies_m9.php?page=$page3'> <i class='icon-arrow-left'></i> Back</a> <a class='btn btn-primary' href='allmovies_m9.php'> <i class='icon-home'></i> Home</a> <a class='btn' href='allmovies_m9.php?page=$page2'>Next <i class='icon-arrow-right'></i></a>";
             }else{
             echo "<a class='btn btn-primary' href='allmovies_m9.php'> <i class='icon-home'></i> Home</a> <a class='btn' href='allmovies_m9.php?page=$page2'>Next <i class='icon-arrow-right'></i></a>";
         } 
         ?>
    </p>
...
$cache_file = "cache/m9_index_d_$page.cache";
if (file_exists($cache_file) && (filemtime($cache_file) > (time() - 60 * 60 * 5 ))) {
  ...
  while ($i < 3) {
     ...
     echo '<tr>';
     for ($y = 0;$y < 4; $y++) {
     echo '<th><a href="#" data-toggle="modal" data-target="#ViewM1_'.$z.'" ><img src="getpostermv_m9.php?id='.$arrXml[$z]['PictureName'].' " width="200" height="200"></br>'.$arrXml[$z]['EnTitle'].'</a></th>';
   $z++;
   }
echo "</tr>";
 $i++;
...
<div id="ViewM1_<?=$p;?>" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  ...
  ?><a href="getlink.php?type=m9&id=<?=$arrXml[$p]['ID'];?>" class="btn btn-primary" target="_BLANK"><i class="icon-film"></i> รับชมซีรี่ส์เรื่องนี้</a><?php
} ?></h3>
        </div>
      <div class="modal-body">
      <p align="center"><img src="getpostermv_m9.php?id=<?=$arrXml[$p]['PictureName'];?>" width="200" height="200"></p>
   ...
   if (checkbookmark($_SESSION['ses_fbid'],$type,$id) == true) { ?>
     <a href="javascript:void()" name="book_<?=$type;?>_<?=$p;?>" id="book_<?=$type;?>_<?=$p;?>" class="btn btn-info" onclick="add('m9',<?=$arrXml[$p]['ID'];?>,'<?=$arrXml[$p]['EnTitle'];?>',<?=$p;?>)"><i class="icon-bookmark"></i> ติดตามเรื่องนี้</a> 
  <?php }else{ ?>
    <a href="javascript:void()" name="book_<?=$type;?>_<?=$p;?>" id="book_<?=$type;?>_<?=$p;?>" class="btn btn-info" onclick="rm('m9',<?=$arrXml[$p]['ID'];?>,<?=$p;?>)"><i class="icon-bookmark"></i> เลิกติดตามเรื่องนี้</a> 
   <?php     }  ?>
        <a href="getlink.php?type=m9&id=<?=$arrXml[$p]['ID'];?>" class="btn btn-primary" target="_BLANK"><i class="icon-film"></i> รับชมซีรี่ส์เรื่องนี้</a>
