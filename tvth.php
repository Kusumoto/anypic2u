...
<center><div class="fb-like-box" data-href="https://www.facebook.com/zxpic" data-width="500" data-height="210" data-show-faces="true" data-stream="false" data-show-border="false" data-header="true"></div></center>
        ...
        <div class="span3">
          <div class="well sidebar-nav">
            <ul class="nav nav-list">
              ...
              <li class="active"><a href="tvth.php">TVTH</a></li>
 ...
 <div class="text-center"><h3>TVTH</h3></div>
 <div>
<?php
$url = "http://allnewservice.serviceseries.com/newversion/tvthService.ashx?";
$data = "type=getall&version=iphone&pid=0&ps=113";

$cache_file = "cache/tvth_index_d_0.cache";
if (file_exists($cache_file) && (filemtime($cache_file) > (time() - 60 * 60 * 5 ))) {
   $result = file_get_contents($cache_file);
	...
	curl_setopt($ch, CURLOPT_USERPWD, "haacked:oqiweuroiuwer");
...
//print_r($output);
$xmlObj = json_decode($result);   
$arrXml = objectsIntoArray($xmlObj);  
//print_r($arrXml);
$i = 0;
while ($i <= 1000) {
	if (!empty($arrXml[$i]['ID'])) {
      if ($vip == 1 || $vip == 2) {
		    echo '<a href="#" onclick="MM_openBrWindow(\'http://anypicplayer.eu5.org/mini.php?url='.base64_encode(bin2hex(base64_encode($arrXml[$i]['ScheduleLink']))).'&pic=getpostermv_m2.php?id='.$pic.'&title='.$arrXml[$i]['EnTitle'].'&part='.$arrXml[$i]['PartName'].'\',\'\',\'status=yes,width=700,height=500\')"><img src="getpostermv_m11.php?id='.$arrXml[$i]['PictureName'].'" width="100px" height="100px"\></a>';
	     } else {
        echo '<a href="javascript:window.alert(\'กรุณาชำระเงินที่หน้า Donate ครับ\')"><img src="getpostermv_m11.php?id='.$arrXml[$i]['PictureName'].'" width="100px" height="100px"\></a>';
       }
  }
	$i++;
}
